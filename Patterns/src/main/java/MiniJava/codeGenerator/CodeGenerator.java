...
public class CodeGenerator {
    ...
    private void defMain() {
        //ss.pop();
        memory.add3AddressCode(ss.pop().num, Operation.JP, new Direct(memory.getCurrentCodeBlockAddress(), varType.Address), null, null);
...
//    }
    ...
    public void pid(Token next) {
        if (symbolStack.size() > 1) {
            ...
            try {
                ...
                ss.push(new Direct(s.address, t));


            } catch (Exception e) {
                ss.push(new Direct(0, varType.Non));
            }
            symbolStack.push(className);
            symbolStack.push(methodName);
        } else {
    ...
    public void intpid(Token next) {
        ss.push(new Immediate(Integer.parseInt(next.value), varType.Int));
    }
    ...
    public void call() {
        ...
        Address temp = new Direct(memory.getTemp(), t);
        ss.push(temp);
        memory.add3AddressCode(Operation.ASSIGN, new Immediate(temp.num, varType.Address), new Direct(symbolTable.getMethodReturnAddress(className, methodName), varType.Address), null);
        memory.add3AddressCode(Operation.ASSIGN, new Immediate(memory.getCurrentCodeBlockAddress() + 2, varType.Address), new Direct(symbolTable.getMethodCallerAddress(className, methodName), varType.Address), null);
...
//        String className = symbolStack.pop();
        try {
            ...
            memory.add3AddressCode(Operation.ASSIGN, param, new Direct(s.address, t), null);
...
//        }
    ...
    public void add() {
        Address temp = new Direct(memory.getTemp(), varType.Int);
        Address s2 = ss.pop();
        Address s1 = ss.pop();
...
//        memory.saveMemory();
    ...
    public void label() {
        ss.push(new Direct(memory.getCurrentCodeBlockAddress(), varType.Address));
    }

    public void save() {
        ss.push(new Direct(memory.saveMemory(), varType.Address));
    }

    public void _while() {
        memory.add3AddressCode(ss.pop().num, Operation.JPF, ss.pop(), new Direct(memory.getCurrentCodeBlockAddress() + 1, varType.Address), null);
        memory.add3AddressCode(Operation.JP, ss.pop(), null, null);
    }

    public void jpf_save() {
        Address save = new Direct(memory.saveMemory(), varType.Address);
        memory.add3AddressCode(ss.pop().num, Operation.JPF, ss.pop(), new Direct(memory.getCurrentCodeBlockAddress(), varType.Address), null);
        ss.push(save);
    }
    ...
    public void equal() {
        Address temp = new Direct(memory.getTemp(), varType.Bool);
        Address s2 = ss.pop();
        Address s1 = ss.pop();
        if (s1.varType != s2.varType) {
    ...
    public void methodReturn() {
        ...
        memory.add3AddressCode(Operation.ASSIGN, s, new Indirect(symbolTable.getMethodReturnAddress(symbolStack.peek(), methodName), varType.Address), null);
